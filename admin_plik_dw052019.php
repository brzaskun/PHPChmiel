...
//bierzemy tylko pierszy sheet bo moga byc pliki w wieloma sheetami
            ...
            if ($nrColumns != 4) {
                echo '<p  style="color: red;">Błąd!. Nieprawidłowa ilość kolumn ' . $nrColumns . '. Mysi być dokładnie 4; Sprawdź ilośc kolumn w pliku i ponownie go załaduj.';
...
//                            }
                        ...
                        //walidacja pci
                        if ($col === 3 && $row != 1) {
                            if (ctype_alpha ($val) || ($val != 1 && $val != 0)) {
                                $blad = array($val, "nieprawidłowe oznaczenie wyboru danych wrażliwych", "w wierszu " . $lp);
                                array_push($wykrytoblad, $blad);
                            }
                        }
                        //walidacja szkolenia
...
//                    );
    ...
    echo sizeof($wykrytoblad) ?> === 0 && <?php echo isset($_FILES["file"]) ? 1 : 0 ?> === 1) {
...
<div class="row" id="panelprzyciskladowanie"  style="box-shadow: 10px 10px 5px #888; padding: 30px; background-color: #e9e9e9;">
        <form id="form2" >
            <input id="zaladuj" type="button" value="Załaduj do bazy"  onclick="uploadusersdw()"
                   style="padding: 10px; width: 180px; margin-top: 4%; margin-left: auto; margin-right: auto; display: none;"/>
            <div id="rezultat">
